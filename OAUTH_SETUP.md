...
# OAuth callbacks must match Google/Meta exactly.
...
- If Google shows `redirect_uri=https://your-domain.comapi/auth/google/callback`, your callback path is missing a `/`.
  Set `BACKEND_URL=https://your-domain.com` and `GOOGLE_CALLBACK_URL=https://your-domain.com/api/auth/google/callback`.
...
4. On Vercel, include `client/vercel.json` so React Router paths like `/auth/callback` are served by `index.html`
5. Set frontend `VITE_API_URL` to your backend URL, for example `https://budget-zen-backe.onrender.com`
6. Use secure session cookies (already configured in `server.js`)
7. Keep your secrets secure - never commit `.env` files

For local OAuth testing, use a matching local pair:
...
For deployed OAuth testing, use a matching deployed pair:

```env
# Vercel frontend env
VITE_API_URL=https://budget-zen-backe.onrender.com

# Render backend env
CLIENT_URL=https://budget-zen-zeta.vercel.app
BACKEND_URL=https://budget-zen-backe.onrender.com
GOOGLE_CALLBACK_URL=https://budget-zen-backe.onrender.com/api/auth/google/callback
```
